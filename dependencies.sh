#!/usr/bin/env bash
# Quick dev/GPU box setup script
# Usage on a fresh machine:
#   curl -fsSL https://your-host.example/setup.sh | bash
# Or:
#   wget -qO- https://your-host.example/setup.sh | bash
#
...
# ─── 3. rclone (for gdrive/onedrive/s3) ──────────────────────────────────────
...
if ! grep -q "dev-setup aliases" ~/.bashrc 2>/dev/null; then
...
echo "  • Set up rclone:  rclone config        (then 'rclone copy file remote:path -P')"
